<header id="header">
	<div class="container">
		<div class="logo fl">
			<img src="img/logo.png" alt="">
		</div>
		<div class="nav-menu fr">
			<div class="nav-trigger fr" id="nav-trigger">
				<i class="fas fa-bars"></i>
			</div>
			<ul class="nav" id="nav-menu">
				<li  id="home-page"><a href="home.php"><i class="fas fa-home"></i>Acasă</a></li>
				<li id="map-page"><a href="#"><i class="fas fa-location-arrow"></i>Hartă</a></li>
				<li id="contact-page"><a href="contact.php"><i class="fas fa-envelope"></i>Contact</a></li>
				<li id="history-page"><a href="#"><i class="fas fa-history"></i>Istoric</a></li>
				<li id="login-page"><a href="login.php"><i class="fas fa-login"></i><i class="fas fa-sign-in-alt"></i>Login</a></li>
			</ul>
		</div>
	</div>
	<div class="clear"></div>
	
</header>

 <script type="text/javascript" src="js/selected-page.js"></script>